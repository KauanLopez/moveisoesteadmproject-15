...
export const useCatalogImagesModal = (catalog: ExternalUrlCatalog) => {
  ...
  const handleUrlSubmit = async (url: string) => {
    ...
    setIsUploading(true);
    try {
      // Re-utiliza a lógica de adicionar uma URL
      const allCatalogs = await externalCatalogService.getAllCatalogs();
      const currentCatalogVersion = allCatalogs.find(c => c.id === catalog.id);
      if (!currentCatalogVersion) throw new Error("Catálogo não encontrado.");

      const updatedUrls = [...(currentCatalogVersion.external_content_image_urls || []), url];
      await externalCatalogService.updateCatalog(catalog.id, { external_content_image_urls: updatedUrls });
  ...
  // As funções toggleFavorite e deleteImage permanecem as mesmas
